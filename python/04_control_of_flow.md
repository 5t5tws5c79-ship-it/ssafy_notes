...
print(dir(math))    # 모듈이 가진 이름들을 리스트로 반환
help(math.sqrt)     # 함수의 독스트링·사용법 확인
```

> 앞서 배운 독스트링이 여기서 빛을 발한다. 모르는 모듈을 만나면 `dir()`과 `help()`로 안을 들여다보자.

### 11.2 for-else

반복문이 **`break` 없이 끝까지 완주했을 때만** `else` 블록이 실행된다.
...
여러 iterable을 **같은 위치끼리 묶어준다.** (지퍼처럼)
...
> 길이가 다르면 **짧은 쪽에 맞춰** 잘린다. `map`에 iterable 여러 개를 넘기는 것과 비슷한 원리다.
